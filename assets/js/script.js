...
const chartBarsHeight = []

for (let i = 0; i < data.length; i++) {
    const dayAmount = data[i].amount;
    const percentOfNum = dayAmount / maxDayAmount * 100;
    chartBarsHeight.push(percentOfNum);
}

setChartBarsHeight(chartBarsHeight)
...
/**
 * --> hide tooltip when leave cursor from the chart bar 
 */

const hideTooltip = function () { tooltip.classList.remove("active") }
